...
export default async function SongDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: review } = await supabase.from("reviews").select("*").eq("song_id", id).single();
  const { data: lenses } = await supabase.from("lens_scores").select("*").eq("song_id", id);
  const { data: defense } = await supabase.from("defense_brief").select("*").eq("song_id", id).single();
